...
const WatchMoviePage = () => {
    ...
    return (
        <div className="main-watch-page">
            ...
            {typeof data !== 'undefined' && data.episode.length === 1 && data.episode[0].url.indexOf("https://ok.ru/") !== -1 && <div style={{display: "flex",justifyContent: "center", marginTop: "20px"}}><iframe title="phim" src={data.episode[0].url} /></div>}
            ...
            {typeof data !== 'undefined' && data.episode.length !== 1 && <div style={{display: "flex", justifyContent: "center", marginTop: "20px"}}>
                {
                    data.episode[param.episode.slice(4,5)-1].url !== '' && data.episode[param.episode.slice(4,5)-1].url.indexOf("https://ok.ru/") !== -1 && <iframe title="phim" src={data.episode[param.episode.slice(4,5)-1].url} />
                }
                {
            ...
            <div className="main-watch-wrap">
                ...
                <ul className="main-watch-episode">
                    ...
                    <div className="main-watch-others">
                        ...
                        <ul>
                            {typeof data2 !== 'undefined' && data2.slice(-4).map(item => {
                                return (
                                    <li>
                ...
                <div className="sidebar">
                    ...
                    <ul className="sidebar-list">
                        {
                            typeof data4 !== 'undefined' && typeof data4.list !== 'undefined' && data4.list.length !== 0 && data4.list.slice(-5).map(item => {
                                return (
                                    <li>
                                        <Link to={`/detail/category=${item.category.replace(/\s/g, '-')}&title=${item.urlTitle}`} style={{backgroundImage: `url(${item.imageUrl})`}} />
                    ...
                    <ul className="sidebar-list">
                    {
                        typeof data3 !== 'undefined' && typeof data3.list !== 'undefined' && data3.list.length !== 0 && data3.list.slice(-5).map(item => {
                            return (
                                <li>
                                    <Link to={`/detail/category=${item.category.replace(/\s/g, '-')}&title=${item.urlTitle}`} style={{backgroundImage: `url(${item.imageUrl})`}} />
                                    <Link to={`/detail/category=${item.category.replace(/\s/g, '-')}&title=${item.urlTitle}`} className="sidebar-item-info">
